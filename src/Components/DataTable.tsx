...
export default function DataTable() {
    ...
    const getData = () => {
      ...
      try {
        fetch(`/ir_data/${specifyFile}?n_rows=${nRows}`).then(response => {
            // if (response.ok){
    ...
    return (
        <div>
            ...
            <Col>
              <Row>
              <button onClick={()=> {getData(); handleCounter()}} className='App-button'>  Refresh table </button>
              ...
              <input  placeholder="file name" type="text" required value={specifyFile} onChange={onFileChangeInput}/>
              <input className="Table-n-rows" placeholder='n rows' type="number" onChange={onnRowsChange} value={nRows}/>
              </Row>
              <Row>
                <div style={{
                display: 'flex',
